...
def _get_and_verify_dtype(
    ...
    **kwargs,
    ...
    """兼容 vLLM 上游不同版本的 _get_and_verify_dtype 调用签名。

    上游可能调用形式:
        1. _get_and_verify_dtype(model_name, hf_config, dtype, **kwargs)
        2. _get_and_verify_dtype(hf_config, dtype, **kwargs)

    为了简单起见，如果位置参数超过 2 个，则视为 (model, hf_config, dtype, ...)。
    我们仅关心 dtype 本身，忽略其它验证逻辑，直接返回解析后的 torch.dtype。
    """

    import torch  # 局部导入避免循环引用

    # 如果有额外位置参数且数量 >=1，说明使用了新签名
    if extra_args:
        # 新签名: (model, hf_config, dtype, ...)
        # 重新映射变量
        model_name = config  # type: ignore
        config = extra_args[0]  # hf_config
        # dtype 位置移位
        dtype = extra_args[1] if len(extra_args) > 1 else dtype  # type: ignore

    # 若 dtype 为 str -> 转成小写
    if isinstance(dtype, str):
        dtype_lower = dtype.lower()
        if dtype_lower in ("auto", "float16", "half"):  # 默认返回 float16
            return torch.float16
        if dtype_lower == "bfloat16":
            return torch.bfloat16
        if dtype_lower in ("float", "float32"):
            return torch.float32

    # 若 dtype 直接是 torch.dtype
