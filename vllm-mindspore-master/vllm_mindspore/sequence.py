...
class Sequence:
    ...
    def prompt_embeds(self) -> Optional[torch.Tensor]:
        return self.inputs.get("prompt_embeds", None)
    ...
    def multi_modal_placeholders(self) -> MultiModalPlaceholderDict:
        return self.inputs.get("multi_modal_placeholders", {})

    @property
    def mm_processor_kwargs(self) -> Dict[str, Any]:
        return self.inputs.get("mm_processor_kwargs", {})
...
class SequenceGroup:
    ...
    def num_finished_seqs(self) -> int:
        """返回已完成的序列数量"""
        return sum(1 for seq in self.seqs if seq.is_finished())

    def is_finished(self) -> bool:
        """判断序列组是否完成"""
        if self.sampling_params.get('use_beam_search', False):
            return self.num_finished_seqs() >= self.sampling_params.get('n', 1)
        else:
