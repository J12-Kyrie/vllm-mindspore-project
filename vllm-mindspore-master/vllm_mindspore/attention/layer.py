...
class Attention(nn.Cell):
    ...
    def fork_kv_cache(
        self,
        parent_key_cache: Tensor,
        parent_value_cache: Tensor,
        parent_slot_mapping: Tensor,
        child_slot_mapping: Tensor,
    ) -> Tuple[Tensor, Tensor]:
        """Fork KV cache for beam search.
        
        Args:
            parent_key_cache: Parent sequence's key cache
            parent_value_cache: Parent sequence's value cache
            parent_slot_mapping: Parent sequence's slot mapping
            child_slot_mapping: Child sequence's slot mapping
            
        Returns:
            Tuple[Tensor, Tensor]: Child sequence's key and value cache
        """
        # 使用 MindSpore 的索引操作复制父序列的 KV Cache
        child_key_cache = ops.gather(parent_key_cache, parent_slot_mapping, 0)
        child_value_cache = ops.gather(parent_value_cache, parent_slot_mapping, 0)
        
        # 将复制的 KV Cache 存储到子序列的位置
        child_key_cache = ops.scatter_update(child_key_cache, child_slot_mapping, child_key_cache)
        child_value_cache = ops.scatter_update(child_value_cache, child_slot_mapping, child_value_cache)
        
        return child_key_cache, child_value_cache
